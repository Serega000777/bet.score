...
Ingestion-контур добавляет `data_provider`, отдельные mapping-таблицы для
соревнований, команд и событий, а также `provider_event_snapshot`. Публичные
UUID не зависят от внешнего API, а каждая версия исходного события сохраняет
checksum и JSON provenance. Подробный контракт описан в
